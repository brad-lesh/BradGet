...
ProgramPage::ProgramPage(Wizard* wizard)
  : Page(wizard)
{
  StaticFrame* tip = new StaticFrame("Welcome to BradGet", this);
  ...
  LinkFrame* link = new LinkFrame("https://github.com/brad-lesh/BradGet", this, ID_LINK);
...
LRESULT ProgramPage::onMessage(uint32 message, WPARAM wParam, LPARAM lParam) {
  ...
  case WM_COMMAND:
    if (LOWORD(wParam) == ID_PROGRAM && HIWORD(wParam) == CBN_SELCHANGE) {
      ...
      ShellExecute(NULL, "open", "https://github.com/brad-lesh/BradGet", NULL, NULL, SW_SHOWNORMAL);
    }
    return 0;
  case WM_TASKDONE:
